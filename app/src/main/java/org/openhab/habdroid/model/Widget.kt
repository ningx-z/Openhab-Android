...
data class Widget(
    ...
    fun toChartUrl(
        ...
        val chartUrl = Uri.Builder()
            ...
            .appendQueryParameter(
                if (item.type === Item.Type.Group) "groups" else "items",
                item.name
            )
    ...
    companion object {
        ...
        fun updateFromEvent(source: Widget, eventPayload: JSONObject): Widget {
            ...
            val icon =
                iconName.toOH2WidgetIconResource(item, source.type, source.mappings.isNotEmpty())
            return Widget(
...
fun Node.collectWidgets(parent: Widget?): List<Widget> {
    ...
    val widget = Widget(
        ...
        labelColor,
        valueColor,
        Widget.sanitizeRefreshRate(refresh),
        actualMin,
        actualMax,
        actualStep,
        Widget.sanitizePeriod(period),
        service,
        null,
        switchSupport,
        height,
